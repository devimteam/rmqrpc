...
type serviceMethod struct {
	...
	argCtxType reflect.Type   // type of the context argument
	argReqType reflect.Type   // type of the request argument
...
func (m *serviceMap) register(rcvr interface{}, name string) error {
	...
	for i := 0; i < s.rcvrType.NumMethod(); i++ {
		...
		if mtype.NumIn() < 3 {
			return fmt.Errorf("rpc: method %s no arguments", method.Name)
		}

		argCtxType := mtype.In(1)

		argReqType := mtype.In(2)
		if argReqType.Kind() != reflect.Ptr || !isExportedOrBuiltin(argReqType) {
			continue
		}
		...
		s.methods[method.Name] = &serviceMethod{
			method:     method,
			argReqType: argReqType.Elem(),
			argCtxType: argCtxType,
		}
	}
